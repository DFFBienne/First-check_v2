...
async function buildPDFBlob(){
  const {blob,filename} = await _buildPDFCore();
...
async function _buildPDFCore(){
  ...
  // 2. BANDEAU INFO — 3 lignes : ligne 1 = Objet + N°Tab | ligne 2 = N°Cpt + CC gen + CC abo | ligne 3 = Fournisseur + Remarques install
  const IH = D.fournisseur || D.remarques_install ? 18*MM : 13*MM;
  const IY=H-HH;
  ...
  // Ligne 1 : Objet + N° tableau
  const ly1 = IY-IH + (D.fournisseur||D.remarques_install ? 13.5*MM : 8.5*MM);
  kv(T.pdfObjet, D.objet, ML, ly1, 55*MM);
  kv(T.pdfNumTab, D.num_tableau, 95*MM, ly1, 30*MM);

  // Ligne 2 : N° compteur + C/C général + C/C abonné
  const ly2 = IY-IH + (D.fournisseur||D.remarques_install ? 8.5*MM : 3.5*MM);
  kv(T.pdfNumCpt, D.num_compteur, ML, ly2, 40*MM);
  kv(T.pdfCcGen, D.cc_general||"—", 75*MM, ly2, 50*MM);
  kv(T.pdfCcAbo, D.cc_abonne||"—", 175*MM, ly2, 45*MM);

  // Ligne 3 (si fournisseur ou remarques_install) : Fournisseur + Remarques install
  if(D.fournisseur||D.remarques_install){
    const ly3 = IY-IH + 3.5*MM;
    if(D.fournisseur) kv(T.pdfFournisseur||'Fournisseur:', D.fournisseur, ML, ly3, 80*MM);
    if(D.remarques_install){
      const remLbl = T.pdfRemarquesInstall||'Remarques:';
      const lw2 = fB.widthOfTextAtSize(remLbl,6.5);
      const remX = D.fournisseur ? 140*MM : ML;
      kv(remLbl, D.remarques_install, remX, ly3, W-remX-MR-lw2-10*MM);
    }
  }
